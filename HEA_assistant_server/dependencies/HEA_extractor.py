...
def chat_chain_get_msg(
    ...
    def truncate_json_by_length(answer4_dict, max_length=5000):
        """
        answer4_dict: dict, e.g. {mat1: {...}, mat2: {...}, ...}
        max_length: int
        ...
        total_len = 2
        truncated = False
        for k, v in answer4_dict.items():
            entry = json.dumps({k: v}, ensure_ascii=False)
            add_len = len(entry) + (1 if out else 0)
            ...
            total_len += add_len
        if truncated:
            out['truncation_info'] = "超过最大输出长度，部分材料未被提取。"
        return out
    if debug:
        del_old = False
        ...
        ba = HEA_assistant(
            system="你是一个严谨的高熵合金材料专家助手, 负责提取文献中的作者研究的材料信息。"
                   "总体目标是抽取每一个独立高熵合金材料的组分、相结构、工艺、性质信息。注意永远以英文回答，"
        ...
        每一个性能均表示成"性能名称":"数值 (单位)，测试条件"，例如"Yield_Strength":"270 (Mpa),1000℃",室温下测量可表示为'RT'，如果文中未描述测试条件则只需填充数值（单位）
        文中未描述的性能记录未'Not Studied'
        输出格式示例如下：
        ```json
        {
           "AlCoCrFeNi2.1_v1": {
            ...
            "ratio_type":"volume_fraction"
            "process_chain":[]
            "Yield_Strength":"270 (Mpa),1000℃"
            "Tensile Strength":"Not Studied"
            },
             "AlCoCrFeNi2.1_v2":{...},
            ...
        }
        ...
        if isinstance(answer4, str):
            try:
                answer4_dict = json.loads(answer4)
            except Exception as e:
                print("answer4 JSON decode error", e)
                answer4_dict = {}
        elif isinstance(answer4, dict):
            answer4_dict = answer4
        else:
            answer4_dict = {}
        max_json_length = 15000
        answer4_trunc = truncate_json_by_length(answer4_dict, max_length=max_json_length)
        # 保存为json字符串，确保闭合
        answer4_json = json.dumps(answer4_trunc, ensure_ascii=False, indent=2)
        # 可选择写回文件备用
        with open(pathlib.Path(out_dir)/"truncated_answer4.json", 'w', encoding='utf-8') as f:
            f.write(answer4_json)
    # 输出json
